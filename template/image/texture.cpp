...
bool Texture::load_texture(const std::string& input_file_name) {
  ...
  // Now convert the image to our required format
  ...
  // We have the converted copy in memmory, we do no need this
...
void Texture::send_to_gpu() {
  // If you have not, ask for a GPU handle
  if (m_texture_id == 0) {
    ask_locations();
  }
  // Bind thsi texture as current
  glBindTexture(GL_TEXTURE_2D, m_texture_id);
  // Send to GPU
  ...
  glGenerateMipmap(GL_TEXTURE_2D); // The GPU generates mipmap
